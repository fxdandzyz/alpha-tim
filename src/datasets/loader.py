...
class DatasetFolder(object):
    ...
    def __getitem__(self, index):
        assert os.path.isfile(os.path.join(self.root+'/'+str(self.data[index])[:9], self.data[index])), os.path.join(self.root+'/'+str(self.data[index])[:9], self.data[index])
        img = Image.open(os.path.join(self.root+'/'+str(self.data[index])[:9], self.data[index])).convert('RGB')
